    ...
    >
      <div className="relative aspect-[3/4] bg-gray-100 overflow-hidden">
        ...
        <div className="absolute top-2 left-2">
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${condition.color}`}>{condition.label}</span>
        </div>
        {stockCount !== undefined && stockCount > 1 && (
          <div className="absolute top-2 right-2">
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-900/70 text-white">{stockCount} in stock</span>
          </div>
        )}
      </div>
      <div className="p-3">
