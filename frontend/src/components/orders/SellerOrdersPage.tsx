 ...
 *
 * Displays all orders for the seller's listings, including buyer info,
 * tracking details, and seller payout information.
 *
 * Requirements: Seller order view
 */
...
/** Seller payout = total_amount - platform_commission - payment_fees (approx 87.5% of total) */
function estimateSellerPayout(totalAmount: number): number {
  // Use listing.seller_payout if available; otherwise approximate
  return Math.round(totalAmount * 0.875 * 100) / 100;
}
...
export default function SellerOrdersPage({ currentUserId, onViewOrder }: SellerOrdersPageProps) {
  ...
  const totalEarnings = completedOrders.reduce(
    (sum, o) => sum + (o.listings?.seller_payout ?? estimateSellerPayout(o.total_amount)),
    0
  );
      ...
      {!loading && completedOrders.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-green-600 mt-0.5">{completedOrders.length} order{completedOrders.length !== 1 ? 's' : ''} delivered</p>
          </div>
      ...
      {!loading && !error && orders.length > 0 && (
        <ul className="space-y-4" aria-label="Sales orders list">
          {orders.map(order => {
            ...
            const payout = order.listings?.seller_payout ?? estimateSellerPayout(order.total_amount);
